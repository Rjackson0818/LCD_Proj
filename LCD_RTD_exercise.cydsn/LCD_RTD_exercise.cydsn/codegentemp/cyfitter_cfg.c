...
#define BS_IOPORT_7_VAL ((const uint8 CYFAR *)0x48000338u)
...
#define BS_IOPINS0_1_VAL ((const uint8 CYFAR *)0x48000354u)

/* IOPINS0_2 Address: CYREG_PRT2_DM0 Size (bytes): 8 */
#define BS_IOPINS0_2_VAL ((const uint8 CYFAR *)0x4800035Cu)

/* CYDEV_CLKDIST_ACFG0_CFG0 Address: CYREG_CLKDIST_ACFG0_CFG0 Size (bytes): 4 */
#define BS_CYDEV_CLKDIST_ACFG0_CFG0_VAL ((const uint8 CYFAR *)0x48000364u)
...
static void AnalogSetDefault(void)
{
	...
	CY_SET_XTND_REG8((void CYFAR *)CYREG_DAC0_SW3, 0x10u);
...
uint8 CYXDATA * const CYCODE ADCMux_CYAMUXSIDE_A__addrTable[4] = {
...
/* This is an implementation detail of the AMux. Code that depends on it may be
   incompatible with other versions of PSoC Creator. */
const uint8 CYCODE ADCMux_CYAMUXSIDE_A__maskTable[4] = {
	0x02u, 0x20u, 
	0x40u, 0x40u, 
};

/*******************************************************************************
* Function Name: ADCMux_CYAMUXSIDE_A_Set
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void ADCMux_CYAMUXSIDE_A_Set(uint8 channel)
{
	if (channel < 2)
	{
		channel += channel;
		*ADCMux_CYAMUXSIDE_A__addrTable[channel] |= ADCMux_CYAMUXSIDE_A__maskTable[channel];
		channel++;
		*ADCMux_CYAMUXSIDE_A__addrTable[channel] |= ADCMux_CYAMUXSIDE_A__maskTable[channel];
	}
}
...
/*******************************************************************************
* Function Name: ADCMux_CYAMUXSIDE_A_Unset
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void ADCMux_CYAMUXSIDE_A_Unset(uint8 channel)
{
	if (channel < 2)
	{
		channel += channel;
		*ADCMux_CYAMUXSIDE_A__addrTable[channel] &= (uint8)~ADCMux_CYAMUXSIDE_A__maskTable[channel];
		channel++;
		*ADCMux_CYAMUXSIDE_A__addrTable[channel] &= (uint8)~ADCMux_CYAMUXSIDE_A__maskTable[channel];
	}
}
...
/* This is an implementation detail of the AMux. Code that depends on it may be
   incompatible with other versions of PSoC Creator. */
uint8 CYXDATA * const CYCODE ADCMux_CYAMUXSIDE_B__addrTable[4] = {
	(uint8 CYXDATA *)CYREG_OPAMP0_SW, (uint8 CYXDATA *)CYREG_OPAMP0_MX, 
	(uint8 CYXDATA *)CYREG_OPAMP2_SW, (uint8 CYXDATA *)CYREG_OPAMP2_MX, 
};

/* This is an implementation detail of the AMux. Code that depends on it may be
   incompatible with other versions of PSoC Creator. */
const uint8 CYCODE ADCMux_CYAMUXSIDE_B__maskTable[4] = {
	0x04u, 0x04u, 
	0x04u, 0x04u, 
};

/*******************************************************************************
* Function Name: ADCMux_CYAMUXSIDE_B_Set
********************************************************************************
* Summary:
*  This function is used to set a particular channel as active on the AMux.
*
...
*
*******************************************************************************/
void ADCMux_CYAMUXSIDE_B_Set(uint8 channel)
{
	if (channel < 2)
	{
		channel += channel;
		*ADCMux_CYAMUXSIDE_B__addrTable[channel] |= ADCMux_CYAMUXSIDE_B__maskTable[channel];
		channel++;
		*ADCMux_CYAMUXSIDE_B__addrTable[channel] |= ADCMux_CYAMUXSIDE_B__maskTable[channel];
	}
}
...
/*******************************************************************************
* Function Name: ADCMux_CYAMUXSIDE_B_Unset
********************************************************************************
* Summary:
*  This function is used to clear a particular channel from being active on the
*  AMux.
...
*
*******************************************************************************/
void ADCMux_CYAMUXSIDE_B_Unset(uint8 channel)
{
	if (channel < 2)
	{
		channel += channel;
		*ADCMux_CYAMUXSIDE_B__addrTable[channel] &= (uint8)~ADCMux_CYAMUXSIDE_B__maskTable[channel];
		channel++;
		*ADCMux_CYAMUXSIDE_B__addrTable[channel] &= (uint8)~ADCMux_CYAMUXSIDE_B__maskTable[channel];
	}
}
...
/* This is an implementation detail of the AMux. Code that depends on it may be
   incompatible with other versions of PSoC Creator. */
uint8 CYXDATA * const CYCODE CurrentMux__addrTable[2] = {
...
/* This is an implementation detail of the AMux. Code that depends on it may be
   incompatible with other versions of PSoC Creator. */
const uint8 CYCODE CurrentMux__maskTable[2] = {
	0x01u, 
	0x20u, 
};

/*******************************************************************************
* Function Name: CurrentMux_Set
********************************************************************************
* Summary:
*  This function is used to set a particular channel as active on the AMux.
*
...
*
*******************************************************************************/
void CurrentMux_Set(uint8 channel)
{
	if (channel < 2)
	{
		*CurrentMux__addrTable[channel] |= CurrentMux__maskTable[channel];
	}
}

/*******************************************************************************
* Function Name: CurrentMux_Unset
********************************************************************************
* Summary:
*  This function is used to clear a particular channel from being active on the
*  AMux.
...
*
*******************************************************************************/
void CurrentMux_Unset(uint8 channel)
{
	if (channel < 2)
	{
		*CurrentMux__addrTable[channel] &= (uint8)~CurrentMux__maskTable[channel];
	}
}
